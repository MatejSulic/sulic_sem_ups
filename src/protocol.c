...
static void close_room_now(Room *r, Game games[], Player players[], const char *reason) {
    ...
    // detach + reset all players bound to this room (connected or reserved for rejoin)
...
static void strike(Player *p, Room rooms[], Game games[], Player players[], const char *msg) {
    ...
    if (p->invalid_count >= MAX_INVALID) {
        ...
        // (Only if arrays are provided; otherwise just reset the player safely.)
        ...
        // Always hard reset this player's slot (frees server capacity)
        player_reset(p);
    }
}
...
static void cmd_join(Player *p, Room rooms[], Game games[], Player players[], int room_id) {
    ...
    if (r->slot_connected[0]) {
        Player *p1 = find_player_by_fd(players, r->player_fds[0]);
        if (p1) {
            snprintf(out, sizeof(out), "JOINED %d 1\nSETUP\n", r->id);
            net_send_all(p1->socket_fd, out);
        }
    }
...
static void destroy_room(Room *r, Game games[], Player players[]) {
    ...
    // detach any connected players (they may still exist in players array)
...
static void cmd_place(Player *p, Room rooms[], Game games[], Player players[], int x, int y, int len, char dir) {
    ...
    char err[64];
    if (!game_place_ship(g, p->player_slot, x, y, len, dir, err, sizeof(err))) {
        char out[96];
        snprintf(out, sizeof(out), "ERROR PLACE %s\n", err);
        net_send_all(p->socket_fd, out);
        strike(p, rooms, games, players, NULL);
        return;
    }

    net_send_all(p->socket_fd, "OK PLACED\n");
...
static void cmd_shoot(Player *p, Room rooms[], Game games[], Player players[], int x, int y) {
    ...
    // send updated boards to both if connected
    for (int slot = 0; slot < 2; slot++) {
    ...
    if (r == 0) {
        ...
        if (p->current_room_id != -1) {
            ...
            if (rm && p->player_slot >= 0) {
                // Only allow "soft" reconnect waiting during setup or active play (shooting).
                ...
                // Any other phase: close the room immediately (no waiting).
