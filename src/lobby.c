...
const char* room_phase_str(RoomPhase ph) {
    return (ph == PHASE_LOBBY)    ? "LOBBY" :
           ...
           (ph == PHASE_PLAY)     ? "PLAY" :
           (ph == PHASE_FINISHED) ? "FINISHED" : "UNKNOWN";
}
...
Room* find_room_by_id(Room rooms[], int room_id) {
    for (int i = 0; i < MAX_ROOMS; i++)
        if (rooms[i].state != ROOM_EMPTY && rooms[i].id == room_id) return &rooms[i];
    return NULL;
}
...
void room_mark_down(Room *r, int slot) {
    ...
    if (r->slot_down_since[slot] == 0) r->slot_down_since[slot] = time(NULL);
    r->player_fds[slot] = -1;
}
...
void lobby_send_room_list(int to_fd, Room rooms[]) {
    int count = 0;
    for (int i = 0; i < MAX_ROOMS; i++)
        if (rooms[i].state != ROOM_EMPTY) count++;
